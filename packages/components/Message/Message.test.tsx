...
describe("Message", () => {
	test("message() function", async () => {
		...
		await rAF();
		expect(document.querySelector(".gg-message")).toBeTruthy();
		handler.close();
		await rAF();
		expect(document.querySelector(".gg-message")).toBeFalsy();
	});
	...
	test("call message() function more than once", async () => {
		...
		await rAF();
		expect(document.querySelectorAll(".gg-message").length).toBe(2);
		closeAll();
		await rAF();
		expect(document.querySelector(".gg-message")).toBeFalsy();
	});

	test("clossAll has type", async () => {
		message({ message: "hello msg", duration: 0 });
		message("hello msg");
		await rAF();
		expect(document.querySelectorAll(".gg-message").length).toBe(2);
	...
	test("message offset", async () => {
		...
		await rAF();
		const elements = document.querySelectorAll(".gg-message");
